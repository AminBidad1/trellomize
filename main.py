...
def check_activation(username: str, view: UserViewSet) -> bool:
    return list(filter(lambda item: item["username"] == username, view.list()))[0].get("is_active")
...
def users_activity(user_id: str):
    view = UserViewSet()
    users = list(map(lambda item: item["username"], view.list()))
    user_activity = dict()
    for user in users:
        user_activity[user] = list(filter(lambda item: item["username"] == user, view.list()))[0].get("is_active")
    print(user_activity)
...
def project_environment(project_id: str):
    ...
    view = UserViewSet()
    console.print("Title : ", list(filter(lambda item: item["id"] == project_id, pview.list()))[0].get("title"))
    leader_id = list(filter(lambda item: item["id"] == project_id, pview.list()))[0].get("leader_id")
    console.print("Leader : ", list(filter(lambda item: item["id"] == leader_id, view.list()))[0].get("username"))
    users = list(filter(lambda item: item["project_id"] == project_id, upview.list()))
    console.print("Users : ")
    for user in users:
        if user.get("user_id") != leader_id:
            console.print(list(filter(lambda item: item["id"] == user.get("user_id"), view.list()))[0].get("username"))
...
def show_projects(user_id: str):
    ...
    for project in projects:
        ...
        if is_ok:
            ...
            console.print(f"{len(user_projects_id)}. {list(filter(lambda item: item["id"] == project.get("project_id"), pview.list()))[0].get("title")}")
...
def show_comments(user_id: str, project_id: str, task_id: str):
    view = CommentViewSet()
    print(view.filter(task_id=task_id))
